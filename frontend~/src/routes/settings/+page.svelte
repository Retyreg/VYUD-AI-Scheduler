<script lang="ts">
	...
	async function addTelegram() {
		...
		try {
			...
			if (!res.ok) { error = d.detail || 'Ошибка'; return; }
			success = 'Telegram аккаунт добавлен!';
	...
	async function addLinkedIn() {
		...
		try {
			...
			const d = await res.json();
			if (!res.ok) { error = d.detail || 'Ошибка'; return; }
			success = 'LinkedIn аккаунт добавлен!';
...
<div class="max-w-screen-xl mx-auto px-4 py-8">
	<div class="mb-6">
		...
		<p class="text-sm text-gray-400 mt-1">Управление подключёнными аккаунтами</p>
	</div>

	<!-- Tabs -->
	<div class="flex gap-2 mb-6 border-b border-gray-800">
		{#each [['accounts', 'Аккаунты'], ['add', 'Добавить аккаунт']] as [tab, label]}
			<button
	...
	{#if activeTab === 'accounts'}
		...
		{:else if accounts.length === 0}
			<div class="text-center py-16">
				...
				<p class="text-gray-400 mb-4">Нет подключённых аккаунтов</p>
				<button on:click={() => activeTab = 'add'} class="text-violet-400 hover:text-violet-300 text-sm">Добавить первый аккаунт →</button>
			</div>
		{:else}
		...
		<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
			...
			<div class="bg-gray-900 border border-gray-800 rounded-2xl p-6">
				...
				<div class="flex flex-col gap-3">
					<input bind:value={tgName} placeholder="Название (например: Мой канал)" class="bg-gray-800 border border-gray-700 rounded-xl px-3 py-2.5 text-sm text-gray-100 outline-none focus:ring-2 focus:ring-blue-500 w-full" />
					<input bind:value={tgToken} placeholder="Bot Token (от @BotFather)" class="bg-gray-800 border border-gray-700 rounded-xl px-3 py-2.5 text-sm text-gray-100 outline-none focus:ring-2 focus:ring-blue-500 w-full font-mono text-xs" />
					<input bind:value={tgChatId} placeholder="Channel ID (например: -100123456)" class="bg-gray-800 border border-gray-700 rounded-xl px-3 py-2.5 text-sm text-gray-100 outline-none focus:ring-2 focus:ring-blue-500 w-full" />
			...
			<div class="bg-gray-900 border border-gray-800 rounded-2xl p-6">
				...
				<div class="flex flex-col gap-3">
					<input bind:value={liName} placeholder="Название" class="bg-gray-800 border border-gray-700 rounded-xl px-3 py-2.5 text-sm text-gray-100 outline-none focus:ring-2 focus:ring-blue-500 w-full" />
			...
			<div class="bg-gray-900 border border-gray-800 rounded-2xl p-6">
				...
				<div class="flex flex-col gap-3">
					<input bind:value={vkName} placeholder="Название" class="bg-gray-800 border border-gray-700 rounded-xl px-3 py-2.5 text-sm text-gray-100 outline-none focus:ring-2 focus:ring-blue-500 w-full" />
					...
					<input bind:value={vkGroupId} placeholder="Group ID (опционально)" class="bg-gray-800 border border-gray-700 rounded-xl px-3 py-2.5 text-sm text-gray-100 outline-none focus:ring-2 focus:ring-blue-500 w-full" />
